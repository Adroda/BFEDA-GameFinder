...
let size = 1440; /*document.querySelector('body').clientWidth*/
...
nextBtn.addEventListener('click', () => {
  ...
  Array.from(carouselBottomWrapper.children).forEach((element, index) => {
    if (index + 1 !== counter) {
      ...
      Array.from(carouselBottomWrapper.children)[counter - 1].classList.add(
...
carouselBottomWrapper.addEventListener('click', (event) => {
  ...
  Array.from(carouselBottomWrapper.children).forEach((element) => {
    if (element !== event.target) {
      ...
      carouselSlide.style.transition = 'transform 0.7s ease-in-out';
      counter = Array.from(carouselBottomWrapper.children).indexOf(element) + 1;
      carouselSlide.style.transform = 'translateX(' + -size * counter + 'px)';
    }
  });
...
window.addEventListener(
    'resize',
    () => (size = document.querySelector('body').clientWidth)
);*/
